...
BEGIN
  ...
  IF v_time_slot_id IS NULL THEN
    ...
    BEGIN
      INSERT INTO time_slots (id, group_id, starts_at, ends_at, created_at)
      VALUES (v_time_slot_id, test_group_id, match_date, match_end, NOW());
      RAISE NOTICE 'Time slot créé avec group_id: %', v_time_slot_id;
    EXCEPTION WHEN undefined_column THEN
      -- Si group_id n'existe pas, créer sans group_id
      INSERT INTO time_slots (id, starts_at, ends_at, created_at)
      VALUES (v_time_slot_id, match_date, match_end, NOW());
  ...
  -- Vérifier que le time_slot a bien été créé/récupéré
  IF v_time_slot_id IS NULL THEN
    RAISE EXCEPTION 'Impossible de créer ou récupérer le time_slot';
  END IF;
