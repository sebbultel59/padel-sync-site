-- Script SQL pour confirmer la participation de Lucas Leroy
...
BEGIN
  ...
  -- 2. Trouver Lucas Leroy
  SELECT p.id INTO v_lucas_id
  FROM profiles p
  WHERE LOWER(p.display_name) LIKE '%lucas%leroy%'
     OR (LOWER(p.name) LIKE '%lucas%' AND LOWER(p.name) LIKE '%leroy%')
  LIMIT 1;
  
  IF v_lucas_id IS NULL THEN
    RAISE EXCEPTION 'Joueur Lucas Leroy non trouvé';
  END IF;
  
  RAISE NOTICE '✅ Lucas Leroy trouvé: %', v_lucas_id;
  
  -- 3. Confirmer la participation
  INSERT INTO match_rsvps (match_id, user_id, status, created_at)
  VALUES (v_match_id, v_lucas_id, 'accepted', NOW())
  ...
  RAISE NOTICE '';
  RAISE NOTICE '✅ Lucas Leroy a confirmé sa participation!';
...
WHERE m.id = 'cc40bdfe-b6c7-4471-9382-bec83a0b58a1'::UUID
  ...
  AND LOWER(p.display_name) LIKE '%lucas%leroy%'
