-- Script pour créer 2 nouveaux matchs validés dans le groupe "50+" avec sebbultel59@gmail.com
-- Status='confirmed' avec 4 joueurs confirmés (dont sebbultel)
-- Semaine du 8 au 14 décembre 2025 (utilise des time_slots non occupés)
...
DECLARE
  ...
  week_start_date DATE := '2025-12-08'; -- Dimanche 8 décembre 2025
...
BEGIN
  ...
  -- Utilise des time_slots non occupés
  -- ==========================================
  RAISE NOTICE '';
  RAISE NOTICE '📋 Création de 2 matchs validés (semaine du 8 au 14 décembre 2025)...';
  
  FOR match_counter IN 1..2 LOOP
    -- Date : 8 décembre + (match_counter - 1) jours, à 18h00
    -- match_counter 1 = 8 déc, 2 = 9 déc
    ...
    IF v_time_slot_id IS NULL THEN
      ...
      EXCEPTION WHEN unique_violation THEN
        ...
        -- Essayer avec un créneau à 19h00 au lieu de 18h00
        match_date := (week_start_date + make_interval(days => match_counter - 1))::date + TIME '19:00:00';
        match_end := match_date + INTERVAL '1 hour 30 minutes';
        ...
        IF v_time_slot_id IS NULL THEN
          ...
          VALUES (v_time_slot_id, test_group_id, match_date, match_end);
          available_slot_found := true;
          RAISE NOTICE '  📅 Nouveau time_slot créé (19h00): % (dates: % -> %)', v_time_slot_id, match_date, match_end;
        END IF;
      END;
    ELSE
      available_slot_found := true;
    ...
    IF NOT available_slot_found THEN
      RAISE EXCEPTION 'Impossible de trouver ou créer un time_slot disponible pour le match #%', match_counter;
    END IF;
  ...
  RAISE NOTICE '   - Semaine du 8 au 14 décembre 2025';
  RAISE NOTICE '   - sebbultel59@gmail.com est inclus dans tous les matchs (confirmé)';
  RAISE NOTICE '   - Time_slots non occupés utilisés';
...
WHERE m.group_id = (SELECT id FROM groups WHERE name ILIKE '%50+%' OR name ILIKE '%test%50%' OR name = 'Groupe de test - 50+' LIMIT 1)
  ...
  AND ts.starts_at >= '2025-12-08'::date
  AND ts.starts_at < '2025-12-15'::date
  AND m.created_at > NOW() - INTERVAL '5 minutes'  -- Seulement les matchs créés récemment
