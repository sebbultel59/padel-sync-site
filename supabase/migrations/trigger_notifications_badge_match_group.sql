-- Migration: Triggers pour les nouvelles notifications (badge_unlocked, match_result_recorded, group_join_request)
-- Date: 2025-01-XX
-- Description: Crée des notification_jobs pour les badges débloqués, résultats de match, et demandes de groupe

-- ============================================================================
-- 1. NOTIFICATION: badge_unlocked
-- ============================================================================
-- Trigger qui crée une notification quand un badge est débloqué
...
-- ============================================================================
-- 2. NOTIFICATION: match_result_recorded
-- ============================================================================
-- Trigger qui crée une notification quand un résultat de match est enregistré
...
-- Créer le trigger
DROP TRIGGER IF EXISTS trigger_notify_match_result_recorded ON match_results;
CREATE TRIGGER trigger_notify_match_result_recorded
  AFTER INSERT OR UPDATE OF status ON match_results
  FOR EACH ROW
  WHEN (NEW.status = 'completed')
  EXECUTE FUNCTION notify_match_result_recorded();

-- ============================================================================
-- 3. NOTIFICATIONS: group_join_request_approved / group_join_request_rejected
-- ============================================================================
-- Trigger qui crée une notification quand une demande de groupe est approuvée ou rejetée
...
-- Créer le trigger
DROP TRIGGER IF EXISTS trigger_notify_group_join_request ON group_join_requests;
CREATE TRIGGER trigger_notify_group_join_request
...
COMMENT ON FUNCTION notify_badge_unlocked() IS 
  'Crée une notification quand un badge est débloqué par un joueur';

COMMENT ON FUNCTION notify_match_result_recorded() IS 
  'Crée une notification pour tous les joueurs quand un résultat de match est enregistré';

COMMENT ON FUNCTION notify_group_join_request() IS 
  'Crée une notification quand une demande de rejoindre un groupe est approuvée ou rejetée';
