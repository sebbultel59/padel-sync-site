-- Match créé depuis « Trouver » : ne pas exiger l’intersection des clubs acceptés (user_clubs)
-- ni l’égalité stricte avec groups.club_id — on ancre le match sur le club porté par la recherche
-- Le club n’est pas filtré par zone (souvent zone_id NULL ou incohérent côté clubs).

DROP FUNCTION IF EXISTS create_match_from_interval_safe(UUID, TIMESTAMPTZ, TIMESTAMPTZ, UUID[], UUID);
...
COMMENT ON FUNCTION create_match_from_interval_safe(UUID, TIMESTAMPTZ, TIMESTAMPTZ, UUID[], UUID, BOOLEAN) IS
  'Crée un match sur un intervalle. Si p_from_find_game, le club (p_club_id) est pris en charge sans règle des clubs communs user_clubs.';
