...
union all
-- Inclure les disponibilités globales seulement s'il n'y a pas d'exception pour ce groupe
select g.user_id,
       gm.group_id,
       g.start,
       g."end",
...
begin
  ...
  if p_status = 'neutral' then
    ...
    if exists (
      ...
      -- Si une disponibilité globale existe, créer une exception "neutral" pour la masquer
      ...
      -- Sinon, supprimer l'exception si elle existe
      delete from availability
      where user_id = p_user
