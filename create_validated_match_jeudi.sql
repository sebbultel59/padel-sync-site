...
-- Avec seb.sax.evenements@gmail.com pour jeudi à 12h00
...
DECLARE
  ...
  player3_id UUID;
  player4_id UUID;
  -- Date du match : jeudi prochain à 12h00
  -- Calculer le prochain jeudi (jour 4 de la semaine ISO, où lundi=1)
  match_date TIMESTAMPTZ := (
    CASE 
      WHEN EXTRACT(DOW FROM CURRENT_DATE) = 0 THEN CURRENT_DATE + INTERVAL '4 days'  -- Si dimanche, jeudi dans 4 jours
      WHEN EXTRACT(DOW FROM CURRENT_DATE) = 1 THEN CURRENT_DATE + INTERVAL '3 days'   -- Si lundi, jeudi dans 3 jours
      WHEN EXTRACT(DOW FROM CURRENT_DATE) = 2 THEN CURRENT_DATE + INTERVAL '2 days'   -- Si mardi, jeudi dans 2 jours
      WHEN EXTRACT(DOW FROM CURRENT_DATE) = 3 THEN CURRENT_DATE + INTERVAL '1 day'    -- Si mercredi, jeudi demain
      WHEN EXTRACT(DOW FROM CURRENT_DATE) = 4 THEN CURRENT_DATE + INTERVAL '7 days'   -- Si jeudi, jeudi prochain (dans 7 jours)
      WHEN EXTRACT(DOW FROM CURRENT_DATE) = 5 THEN CURRENT_DATE + INTERVAL '6 days'  -- Si vendredi, jeudi dans 6 jours
      WHEN EXTRACT(DOW FROM CURRENT_DATE) = 6 THEN CURRENT_DATE + INTERVAL '5 days'  -- Si samedi, jeudi dans 5 jours
    END
  )::date + TIME '12:00:00';
...
BEGIN
  ...
  -- 3. Créer ou récupérer un time_slot pour jeudi prochain à 12h00 (1h30)
